...
class TicketDAO
{
    public function create(Ticket $ticket)
    {
        ...
        $stmt = $conn->prepare("INSERT INTO ticket (id, sorteio, cpf, numeros_apostados) VALUES (?, ?, ?, ?)");
        $stmt->bindValue(1, $ticket->getId());
        $stmt->bindValue(2, $ticket->getSorteio()->getId());
        $stmt->bindValue(3, $ticket->getCpf());
    ...
    {
        $conn = Conexao::getConn();
        $stmt = $conn->prepare("UPDATE ticket SET sorteio = ?, cpf = ?, numeros_apostados = ? WHERE id = ?");
        $stmt->bindValue(1, $ticket->getSorteio()->getId());
        $stmt->bindValue(2, $ticket->getCpf());
